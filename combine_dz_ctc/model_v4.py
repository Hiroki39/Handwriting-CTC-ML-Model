...
cfg_model_path = './output'
cfg_ini_model_path = None
...
if (platform.system()[0] == 'W'):
    ...
    # os.path.join('./test_data', os.listdir('./test_data')[0])
    cfg_hz_code_path = None
    #cfg_hz_feat_path = os.path.join('./train_data', "hw_dz_data.bin")
    cfg_hz_feat_path = os.path.join('./train_data', "ctc_train_data.bin")
    print(cfg_hz_feat_path)

train_place = None
data_reader = ds.Dataset()
#data_reader.open(cfg_hz_code_path, cfg_hz_feat_path, cfg_batch_size)
...
def model(inputs, training=True):
    ...
    for i in range(cfg_lstm_num):
        ...
        lstm_fw, _ = lstm_cell(
            inputs, cfg_lstm_dim, cfg_lstm_use_peepholdes, False, "fw_lstm_" + str(i))
        lstm_bw, _ = lstm_cell(
            inputs, cfg_lstm_dim, cfg_lstm_use_peepholdes, True, "bw_lstm_" + str(i))
        inputs = fluid.layers.concat([lstm_fw, lstm_bw], axis=1)
        inputs = fluid.layers.fc(inputs, cfg_lstm_dim,
                                 act="relu", name="lstm_sf_" + str(i))
    ...
    #inputs = fluid.layers.fc(inputs, cfg_fc_dim, act="relu", name="fc")
    ctc_outputs = fluid.layers.fc(inputs, cfg_ctc_output_dim, name="ctc_fc")

    nm_output, _ = lstm_cell(
        inputs, cfg_fc_dim, cfg_lstm_use_peepholdes, True, "nm_lstm")
    nm_output = fluid.layers.sequence_pool(nm_output, "first")

    tst_output = ctc_outputs
...
def train_ctc():
    ...
    _, ctc_output = model(inputs, True)
    ...
    for epoch in range(cfg_max_epoch):
...
# save_infer_model('D:\\_HW\\model_v4_1_dz\\2', 'D:\\_HW\\model_v4_1_dz\\2_infer')
