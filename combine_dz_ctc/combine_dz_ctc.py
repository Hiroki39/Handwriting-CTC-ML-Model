...
cfg_ini_model_path = './output/31'
cfg_backbone_trainable = True
...
if (platform.system()[0] == 'W'):
    ...
    cfg_hz_feat_path = os.path.join('./train_data', 'hw_dz_data.debug.bin')
...
def train_ctc():
    ...
    nm_output, ctc_outputs = model(inputs, True)

    ctc_loss = fluid.layers.warpctc(
        input=ctc_outputs, label=fluid.layers.cast(labels, "int32"))

    ce_loss = fluid.layers.softmax_with_cross_entropy(nm_output, label=labels)

    loss = cfg_ctc_w * ctc_loss + cfg_ce_w * ce_loss
